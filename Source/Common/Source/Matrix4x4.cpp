...
namespace Jam
{
	...
	Matrix4x4 &Matrix4x4::Rotate( const JAM_FLOAT32 p_Angle,
		const Vector4 &p_Axis )
	{
		JAM_FLOAT32 Sin = 0.0f, Cos = 0.0f, Tan = 0.0f;
		Jam::SineCosine( p_Angle, Sin, Cos );
	...
	Matrix4x4 &Matrix4x4::RotateX( const JAM_FLOAT32 p_Angle )
	{
		JAM_FLOAT32 Sin = 0.0f, Cos = 0.0f;
		Jam::SineCosine( p_Angle, Sin, Cos );
	...
	Matrix4x4 &Matrix4x4::RotateY( const JAM_FLOAT32 p_Angle )
	{
		JAM_FLOAT32 Sin = 0.0f, Cos = 0.0f;
		Jam::SineCosine( p_Angle, Sin, Cos );
	...
	Matrix4x4 &Matrix4x4::RotateZ( const JAM_FLOAT32 p_Angle )
	{
		JAM_FLOAT32 Sin = 0.0f, Cos = 0.0f;
		Jam::SineCosine( p_Angle, Sin, Cos );
	...
	Matrix4x4 &Matrix4x4::Scale( const JAM_FLOAT32 p_Scale )
	{
	...
	Matrix4x4 &Matrix4x4::ScaleX( const JAM_FLOAT32 p_Scale )
	{
		this->Identity( );
		m_M[ 0 ] = p_Scale;
	...
	Matrix4x4 &Matrix4x4::ScaleY( const JAM_FLOAT32 p_Scale )
	{
		this->Identity( );
		m_M[ 5 ] = p_Scale;
	...
	Matrix4x4 &Matrix4x4::ScaleZ( const JAM_FLOAT32 p_Scale )
	{
		this->Identity( );
		m_M[ 10 ] = p_Scale;
	...
	Matrix4x4 &Matrix4x4::CreatePerspectiveFOV( const JAM_FLOAT32 p_FOV,
		const JAM_FLOAT32 p_AspectRatio, const JAM_FLOAT32 p_NearPlane,
		const JAM_FLOAT32 p_FarPlane )
	{
		if( Jam::Absolute( p_FarPlane - p_NearPlane ) < JAM_EPSILON )
		{
			return *this;
		}
		...
		JAM_FLOAT32 D = 1.0f / tanf( p_FOV / 180.0f ) * JAM_PI * 0.5f;
		JAM_FLOAT32 Recip = 1.0f / ( p_NearPlane - p_FarPlane );
	...
	JAM_FLOAT32 Matrix4x4::Trace( ) const
	{
	...
	JAM_FLOAT32 &Matrix4x4::operator[ ]( const JAM_MEMSIZE p_Index )
	{
		return m_M[ p_Index ];
	}

	JAM_FLOAT32 Matrix4x4::operator[ ]( const JAM_MEMSIZE p_Index ) const
	{
		return m_M[ p_Index ];
	}


	void Matrix4x4::GetAsFloatArray( JAM_FLOAT32 p_Raw[ 16 ] ) const
	{
