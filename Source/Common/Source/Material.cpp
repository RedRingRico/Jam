...
namespace Jam
{
	...
	JAM_UINT32 Material::GetDigest( MD5_DIGEST &p_Digest ) const
	{
	...
	JAM_UINT32 Material::CreateFromFile( const std::string &p_FileName )
	{
		if( p_FileName.size( ) == 0 )
		{
			std::cout << "[Jam::Material::CreateFromFile] <ERROR> "
		...
		JAM_MEMSIZE FileSize = ftell( pMaterialFile );
		...
		{
			std::cout << "[Jam::Material::CreateFromFile] <INFO> "
			...
			if( ShaderRoot.HasMember( "source" ) )
			{
					...
					++i )
				{
					...
					if( ShaderSourceRoot[ i ].HasMember( "type" ) )
					{
						...
						{
							std::cout << "[Jam::Material::CreateFromFile] "
		...
		{
			std::cout << "[Jam::Material::CreateFromFile] <INFO> No textures "
				"present" << std::endl;
		}
	...
	JAM_UINT32 Material::GetShader( MD5_DIGEST &p_Digest ) const
	{
	...
	JAM_UINT32 Material::GetTextures(
		std::vector< MD5_DIGEST > &p_Digests ) const
	{
		for( JAM_MEMSIZE TextureIndex = 0;
