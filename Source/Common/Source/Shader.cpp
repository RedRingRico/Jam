...
namespace Jam
{
	...
	JAM_UINT32 Shader::CreateShaderFromSource( const std::string &p_Source,
		const SHADER_TYPE p_ShaderType, const JAM_BOOL p_FromFile )
	{
		if( p_Source.size( ) == 0 )
		{
			std::cout << "[Jam::Shader::CreateShaderFromSource] <ERROR> "
		...
		GLchar *pSource = JAM_NULL;
		GLint SourceLength = 0;

		if( p_FromFile == JAM_FALSE )
		{
		...
		if( this->ExtractUniformNames( pSource ) != JAM_OK )
		{
			std::cout << "[Jam::Shader::CreateShaderFromSource] <ERROR> "
	...
	JAM_UINT32 Shader::Apply( )
	{
		if( m_Linked == JAM_FALSE )
		{
			if( this->Link( ) != JAM_OK )
			{
				return JAM_FAIL;
	...
	JAM_UINT32 Shader::SetShaderParameter( const std::string &p_Name,
		void *p_pData )
	{
		...
		{
			std::cout << "[Jam::Shader::SetShaderParameter] <ERROR> Could not "
	...
	JAM_UINT32 Shader::GetShaderParameters(
	...
	JAM_UINT32 Shader::GetDigest( MD5_DIGEST &p_Digest ) const
	{
		if( m_Linked )
		{
	...
	JAM_UINT32 Shader::ExtractUniformNames( const GLchar *p_pSource )
	{
		...
		JAM_MEMSIZE UniformPosition = SourceCopy.find( "uniform" );

		while( UniformPosition != std::string::npos )
		{
			// Get the first set of non-whitespace characters before the semi-
			// colon
			JAM_MEMSIZE DelimiterPosition = SourceCopy.find_first_of( ",;",
				UniformPosition );

			if( DelimiterPosition == std::string::npos )
			{
				std::cout << "[Jam::Shader::ExtractUniformNames] <ERROR> "
			...
			// Step 1: Get the type
			JAM_MEMSIZE TypeStart = SourceCopy.find_first_of( " \t",
				UniformPosition );
			// Get rid of any leading whitespace
			TypeStart = SourceCopy.find_first_not_of( " \t", TypeStart );
			JAM_MEMSIZE TypeEnd = SourceCopy.find_first_of( " \t", TypeStart );
			...
			JAM_MEMSIZE UniformNameStart = SourceCopy.find_first_not_of( " \t",
				TypeEnd );
			UniformNameStart = SourceCopy.find_first_not_of( " \t",
				UniformNameStart );
			JAM_MEMSIZE UniformNameEnd = SourceCopy.find_first_of( ", [;\t",
				UniformNameStart );
			JAM_MEMSIZE CommaLocation = SourceCopy.find_first_of( ",",
				UniformNameStart );
			JAM_MEMSIZE EndOfLine = SourceCopy.find_first_of( ';',
				UniformNameStart );

			JAM_BOOL DoneProcessing = JAM_FALSE;
			...
			do
			{
				...
				JAM_MEMSIZE ArrayStart = SourceCopy.find_first_of( '[',
					UniformNameEnd );

				JAM_MEMSIZE LastItem = SourceCopy.find_first_not_of( " \t",
					UniformNameEnd );
				JAM_MEMSIZE ArrayEnd = 0;
				JAM_BOOL FoundArray = JAM_FALSE;
					...
					( ArrayStart != std::string::npos ) )
				{
						...
						( ArrayStart < CommaLocation ) )
					{
						...
						{
							JAM_MEMSIZE ArrayEndTmp = SourceCopy.find_first_of(
				...
				{
					std::cout << "[Jam::Shader::ExtractUniformNames] <WARN> "
				...
				CommaLocation = SourceCopy.find_first_of( ",",
					UniformNameStart );
			}while( DoneProcessing == JAM_FALSE );
	...
	JAM_UINT32 Shader::Link( )
	{
		...
		{
			std::cout << "[Jam::Shader::Link] <ERROR> Unable to create shader "
				"program" << std::endl;
		...
		if( Error == GL_FALSE )
		{
			...
			std::cout << "[Jam::Shader::Link] <ERROR> Failed to link program:"
				<< std::endl;
		...
		if( this->BindUniformNamesToLocations( ) != JAM_OK )
		{
			return JAM_FAIL;
		}
	...
	JAM_UINT32 Shader::BindUniformNamesToLocations( )
	{
		...
		while( UniformItr != m_UniformLocationMap.end( ) )
		{
			...
			switch( Location )
			{
				...
				{
					std::cout << "[Jam::Shader::BindUniformNamesToLocations] "
